...
!function(){"use strict";function nospace(){return function(value){return value?value.replace(/ /g,""):""}}function humanizeDoc(){return function(doc){if(doc)return"directive"===doc.type?doc.name.replace(/([A-Z])/g,function($1){return"-"+$1.toLowerCase()}):doc.label||doc.name}}angular.module("core").filter(nospace).filter(humanizeDoc)}();
...
!function(){"use strict";function HeaderController($scope,$state,Authentication,menuService,$mdSidenav){function stateChangeSuccess(){$mdSidenav("left").close()}function toggleList(){$mdSidenav("left").toggle()}function isOpen(menuState){return menuService.isSectionSelected(menuState)}function toggleOpen(menuState){menuService.toggleSelectSection(menuState)}var vm=this;vm.accountMenu=menuService.getMenu("account").items[0],vm.authentication=Authentication,vm.menu=menuService.getMenu("topbar"),vm.toggleList=toggleList,vm.isOpen=isOpen,vm.toggleOpen=toggleOpen,$scope.$on("$stateChangeSuccess",stateChangeSuccess)}angular.module("core").controller("HeaderController",HeaderController),HeaderController.$inject=["$scope","$state","Authentication","menuService","$mdSidenav"]}();
!function(){"use strict";function HomeController($mdSidenav){}angular.module("core").controller("HomeController",HomeController),HomeController.$inject=["$mdSidenav"]}();
!function(){"use strict";function pageTitle($rootScope,$timeout,$interpolate,$state){function link(scope,element){function listener(event,toState){var title=getTitle($state.$current);$timeout(function(){element.text(title)},0,!1)}function getTitle(currentState){var applicationCoreTitle="MEAN.js",workingState=currentState;if(currentState.data){workingState="undefined"!=typeof workingState.locals?workingState.locals.globals:workingState;var stateTitle=$interpolate(currentState.data.pageTitle)(workingState);return applicationCoreTitle+" - "+stateTitle}return applicationCoreTitle}$rootScope.$on("$stateChangeSuccess",listener)}var directive={restrict:"A",link:link};return directive}angular.module("core").directive("pageTitle",pageTitle),pageTitle.$inject=["$rootScope","$timeout","$interpolate","$state"]}();
